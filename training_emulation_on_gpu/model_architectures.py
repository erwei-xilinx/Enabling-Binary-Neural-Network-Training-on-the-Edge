...
def get_model(dataset,resid_levels,batch_size):
	if dataset=='MNIST':
		model=Sequential()
		model.add(binary_dense(n_in=784,n_out=64,input_shape=[784],first_layer=True))
		# model.add(BatchNormalization(axis=-1, momentum=batch_norm_momentum, epsilon=batch_norm_eps))
		model.add(l1_batch_norm_mod_dense(batch_size=batch_size,ch_in=64,momentum=batch_norm_momentum))
		model.add(binary_activation())
		model.add(binary_dense(n_in=int(model.output.get_shape()[1]),n_out=64))
		# model.add(BatchNormalization(axis=-1, momentum=batch_norm_momentum, epsilon=batch_norm_eps))
		model.add(l1_batch_norm_mod_dense(batch_size=batch_size,ch_in=64,momentum=batch_norm_momentum))
		model.add(binary_activation())
		model.add(binary_dense(n_in=int(model.output.get_shape()[1]),n_out=64))
